...
RDEPEND+="
	!www-client/chromium
	=www-plugins/inox-binary-plugins-${PV}*[widevine?]
	x11-misc/xdg-utils
	virtual/opengl
	virtual/ttf-fonts
...
src_install() {
	...
	if [[ -n ${CHROMIUM_SUFFIX} ]]; then
		sedargs+=(
			-e "s:inox-browser:inox-browser${CHROMIUM_SUFFIX}:g"
			-e "s:inox.desktop:inox${CHROMIUM_SUFFIX}.desktop:g"
			-e "s:plugins:plugins --user-data-dir=\${HOME}/.config/inox${CHROMIUM_SUFFIX}:"
		)
	fi
	...
	dosym "${CHROMIUM_HOME}/inox-launcher.sh" /usr/bin/chromium-browser${CHROMIUM_SUFFIX} || die
	...
	dodir /etc/inox || die
	insinto /etc/inox
	newins "${FILESDIR}/inox.default" "default" || die
