...
#include "cmtreestoresort.h"
#include "gtktreedatalistsort.h"
...
#define CM_TREE_STORE_IS_SORTED(tree) (((CMTreeStore*)(tree))->priv->sort_column_id != GTK_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID)
...
static void         cm_tree_store_sortable_init   (GtkTreeSortableIface   *iface);
...
static void     cm_tree_store_sort                    (CMTreeStore           *tree_store);
static void     cm_tree_store_sort_iter_changed       (CMTreeStore           *tree_store,
							GtkTreeIter            *iter,
							gint                    column,
							gboolean                emit_signal);
static gboolean cm_tree_store_get_sort_column_id      (GtkTreeSortable        *sortable,
							gint                   *sort_column_id,
							GtkSortType            *order);
static void     cm_tree_store_set_sort_column_id      (GtkTreeSortable        *sortable,
							gint                    sort_column_id,
							GtkSortType             order);
static void     cm_tree_store_set_sort_func           (GtkTreeSortable        *sortable,
							gint                    sort_column_id,
							GtkTreeIterCompareFunc  func,
							gpointer                data,
							GDestroyNotify          destroy);
static void     cm_tree_store_set_default_sort_func   (GtkTreeSortable        *sortable,
							GtkTreeIterCompareFunc  func,
							gpointer                data,
							GDestroyNotify          destroy);
static gboolean cm_tree_store_has_default_sort_func   (GtkTreeSortable        *sortable);
...
static void
cm_tree_store_sortable_init (GtkTreeSortableIface *iface)
{
  iface->get_sort_column_id = cm_tree_store_get_sort_column_id;
  iface->set_sort_column_id = cm_tree_store_set_sort_column_id;
  iface->set_sort_func = cm_tree_store_set_sort_func;
  iface->set_default_sort_func = cm_tree_store_set_default_sort_func;
  iface->has_default_sort_func = cm_tree_store_has_default_sort_func;
}
			       ...
			       gboolean      sort)
{
  ...
  while (list != NULL)
      ...
      if (column == 0)
	{
	  ...
	  if (converted)
	    g_value_unset (&real_value);
          if (sort && CM_TREE_STORE_IS_SORTED (tree_store))
            cm_tree_store_sort_iter_changed (tree_store, iter, old_column, TRUE);
	  return retval;
	}
			  ...
			  GValue       *value)
{
  ...
  if (cm_tree_store_real_set_value (tree_store, iter, column, value, TRUE))
    {
      GtkTreePath *path;
...
cm_tree_store_get_compare_func (CMTreeStore *tree_store)
{
  CMTreeStorePrivate *priv = tree_store->priv;
  GtkTreeIterCompareFunc func = NULL;

  if (CM_TREE_STORE_IS_SORTED (tree_store))
    {
				    ...
				    gint          n_values)
{
  ...
  func = cm_tree_store_get_compare_func (tree_store);
			    ...
			    gint          n_values)
{
  ...
  if (maybe_need_sort && CM_TREE_STORE_IS_SORTED (tree_store))
    cm_tree_store_sort_iter_changed (tree_store, iter, priv->sort_column_id, TRUE);
				   ...
				   ...)
{
  ...
  if (maybe_need_sort && CM_TREE_STORE_IS_SORTED (tree_store))
    cm_tree_store_sort_iter_changed (tree_store, iter, priv->sort_column_id, FALSE);
				  ...
				  GtkSelectionData *selection_data)
{
  ...
  if (CM_TREE_STORE_IS_SORTED (drag_dest))
    return FALSE;
			...
			gint         *new_order)
{
  ...
  g_return_if_fail (CM_IS_TREE_STORE (tree_store));
  g_return_if_fail (!CM_TREE_STORE_IS_SORTED (tree_store));
...
/* Sorting */
static gint
cm_tree_store_compare_func (gconstpointer a,
			     gconstpointer b,
			     gpointer      user_data)
{
  CMTreeStore *tree_store = user_data;
  CMTreeStorePrivate *priv = tree_store->priv;
  GNode *node_a;
  GNode *node_b;
  GtkTreeIterCompareFunc func;
...
static void
cm_tree_store_sort_helper (CMTreeStore *tree_store,
			    GNode        *parent,
			    gboolean      recurse)
{
  GtkTreeIter iter;
  ...
  if (node == NULL || node->next == NULL)
    {
      if (recurse && node && node->children)
        cm_tree_store_sort_helper (tree_store, node, TRUE);
  ...
  /* Sort the array */
  g_array_sort_with_data (sort_array, cm_tree_store_compare_func, tree_store);
  ...
  if (recurse)
      ...
      for (tmp_node = parent->children; tmp_node; tmp_node = tmp_node->next)
	{
	  if (tmp_node->children)
	    cm_tree_store_sort_helper (tree_store, tmp_node, TRUE);
	}
    }
}

static void
cm_tree_store_sort (CMTreeStore *tree_store)
{
  CMTreeStorePrivate *priv = tree_store->priv;
  ...
  cm_tree_store_sort_helper (tree_store, G_NODE (priv->root), TRUE);
}

static void
cm_tree_store_sort_iter_changed (CMTreeStore *tree_store,
				  GtkTreeIter  *iter,
				  gint          column,
				  gboolean      emit_signal)
{
  ...
  tmp_path = cm_tree_store_get_path (GTK_TREE_MODEL (tree_store), &tmp_iter);
...
static gboolean
cm_tree_store_get_sort_column_id (GtkTreeSortable  *sortable,
				   gint             *sort_column_id,
				   GtkSortType      *order)
{
  CMTreeStore *tree_store = (CMTreeStore *) sortable;
  CMTreeStorePrivate *priv = tree_store->priv;
...
static void
cm_tree_store_set_sort_column_id (GtkTreeSortable  *sortable,
				   gint              sort_column_id,
				   GtkSortType       order)
{
  CMTreeStore *tree_store = (CMTreeStore *) sortable;
  ...
  cm_tree_store_sort (tree_store);
}

static void
cm_tree_store_set_sort_func (GtkTreeSortable        *sortable,
			      gint                    sort_column_id,
			      GtkTreeIterCompareFunc  func,
			      gpointer                data,
			      GDestroyNotify          destroy)
...
static void
cm_tree_store_set_default_sort_func (GtkTreeSortable        *sortable,
				      GtkTreeIterCompareFunc  func,
				      gpointer                data,
				      GDestroyNotify          destroy)
{
...
static gboolean
cm_tree_store_has_default_sort_func (GtkTreeSortable *sortable)
{
  CMTreeStore *tree_store = (CMTreeStore *) sortable;
