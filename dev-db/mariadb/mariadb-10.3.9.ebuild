...
SLOT="0/${SUBSLOT:-0}"
IUSE="+backup bindist client-libs cracklib debug embedded extraengine galera innodb-lz4
...
{
	local f
	pushd "${MY_PATCH_DIR}" >/dev/null
		for f in *without-clientlibs*.patch;
		do
			einfo adjust ${f}
			sed '\#^diff -aurN a/extra/CMakeLists.txt#,\#^diff#{//!d}' -i ${f}
		done
	popd >/dev/null
}
